...
if __name__ == "__main__":
    ...
    # 动态提取所有 Tensor 类型的参数并转为 numpy
    # 这样可以保留 body_pose, betas, global_orient, transl, jaw_pose, hand_pose 等所有数据
    smpl_dict_numpy = {k: v.numpy() for k, v in smpl_params.items() if torch.is_tensor(v)}
    
    # 手动添加 gender 字段，GVHMR 默认通常使用 neutral (中性)
    smpl_dict_numpy['gender'] = 'neutral' 
    
    np.savez(smplx_npz_path, **smpl_dict_numpy)
    Log.info(f"[Save SMPLX] Successfully saved ALL params to {smplx_npz_path}")